...
def generate_many_test_cases[Input, Output](
    ...
    verify_solution: Callable[[Input, Output], None],
    max_solve_seconds: float = 1,
    over_sampling: float = 1,
):
    cases = []
    for i in range(int(len(the_range) * over_sampling)):
        while True:
            try:
    ...
    print("How many candidate problems do we have? ", len(cases))
    for (i, i_in_the_range) in enumerate(the_range):
        the_case = cases[int(i * over_sampling)]
        test_path = f"{test_prefix}{i_in_the_range}"
